            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header border-bottom">
                        <h3 class="card-title">لیست نمونه کارها</h3>
                    </div>
                    <div class="card-body">
                            ...
                            <table class="table table-bordered text-nowrap border-bottom w-100" id="responsive-datatable">
                                ...
                                @foreach($items as $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->title }}</td>
                                        <td>
                                            <a href="{{ route('portfolios.edit', $item->id) }}" class="btn btn-primary fs-14 text-white edit-icn" title="ویرایش">
                                            ...
                                            <form id="form-{{ $item->id }}" action="{{ route('portfolios.destroy', $item->id) }}" method="post">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </td>
                    ...
                    <div class="card-footer">
                        <a href="{{ route('portfolios.create') }}" class="btn btn-primary">افزودن</a>
                    </div>
                </div>
            </div>
        </div>
