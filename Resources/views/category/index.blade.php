            ...
            <div class="col-lg-12">
                <div class="card">
                                    ...
                                    @foreach($categories as $category)
                                        <li class="dd-item" draggable="true" data-id="{{ $category->id }}">
                                            ...
                                            <div class="btn-inline">
                                                <a class="btn btn-primary" href="{{ route('PortfolioCategory.edit', $category->id) }}" title="ویرایش"><i class="fa fa-edit"></i></a>
                                                <form action="{{ route('PortfolioCategory.destroy', $category->id) }}" method="post" class="delete-form">
                                                    <button class="delete btn btn-danger" onclick="return confirm('برای حذف اطمینان دارید؟')"><i class="fa fa-trash"></i></button>
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                            </div>
                                            @include('portfolio::category.each', $category)
                                        </li>
                                    @endforeach
                                </ol>
                            </div>
                    ...
                    <div class="card-footer">
                        <a href="{{ route('PortfolioCategory.create') }}" class="btn btn-primary">افزودن دسته بندی</a>
                    </div>
                </div>
            </div>
        </div>
